...
std::vector<std::string> CPU_Chip8::GetRegStrings()
{
	...
	int instructions_to_dissassemble = 10;
	...
	for (int i = 0; i <= instructions_to_dissassemble; i++) {
		...
		dis.push_back(fmt::format("|  0x{2:04x} : {0:04X} - {1:25s}", opcode, dis_string, next_pc));
		next_pc += 2;
	}
...
{
	///@todo Implement
	g_log->Warn("CHIP8", "Draw not implemented yet! (pc {0:04x})", cpu->regs.pc);
	cpu->regs.pc += 2;
}
